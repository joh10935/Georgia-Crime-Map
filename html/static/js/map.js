...
function createMap(county) {
  ...
  legend.onAdd = function () {
    ...
    departmentColor = ["red", "yellow", "green"];
    div.innerHTML = "<h3>Crime Solved Rate</h3>";
    for (a = 0; a < departmentIntervals.length; a++) {
      div.innerHTML += "<i style='background: " + departmentColor[a] + "'></i>" + departmentIntervals[a] +
        (departmentIntervals[a + 1] ? " &ndash; " + departmentIntervals[a + 1] + " Percent" + " <br>": "+");
    }
    ...
    var colors = ["#FF0000", "#ff4500", "#00FFFF", "#008000"];
...
d3.json("../data/crimes_location.json").then(function (crimes) {
  ...
  d3.json("../data/Ga_Department_Locations.json").then(function (countydata) {
      ...
      for (i = 0; i < countydata.length; i++) {
        ...
        if (_.findIndex(crimedct, { loc: countydata[i]["Agency Name"] }) >= 0)
          locArea = L.marker([lat, lng], {icon: crimedct[_.findIndex(crimedct, { loc: countydata[i]["Agency Name"] })].markerColor,}).bindPopup(
            `<h2> ${countydata[i]["Agency Name"]}</h2><hr> <p>Total crimes: ${crimedct[_.findIndex(crimedct, { loc: countydata[i]["Agency Name"] })].actual
            } </p>Solved Crimes: ${crimedct[_.findIndex(crimedct, { loc: countydata[i]["Agency Name"] })].clear}</p><p> Percent Solved: ${
              crimedct[_.findIndex(crimedct, { loc: countydata[i]["Agency Name"] })].percent}%`);
        else
          locArea = L.marker([lat, lng], { icon: purpleMarker }).bindPopup(`<h2> ${countydata[i]["Agency Name"]}</h2>`);
    ...
    d3.json("../data/ID_capita.json").then(function (income) {
      ...
      //County Crime data
      d3.json("../data/crimes_county.json").then(function (crimelocation) {
        cldict = [];
          ...
          var county = L.geoJson(geojson, {
            ...
            onEachFeature: function (Feature, layer) {
              ...
              if (_.findIndex(cldict, {countloc: _.toUpper(Feature.properties.NAME),}) >= 0)
                layer.bindPopup(`<h2>${Feature.properties.NAME} County </h2> <hr> <p>2020 Median Income: $${combined[_.findIndex(combined, {cty: _.toUpper(Feature.properties.NAME) + " COUNTY",})].inc}</p> 
                <p>Total Crimes: ${cldict[_.findIndex(cldict, {countloc: _.toUpper(Feature.properties.NAME),})].countactual}</p> <p>Solved Crimes: ${cldict[_.findIndex(cldict, {countloc: _.toUpper(Feature.properties.NAME),})].countclear}</p>
                <p>Percent of Crimes Solved: ${cldict[_.findIndex(cldict, {countloc: _.toUpper(Feature.properties.NAME),})].countpercent}%`);
              else 
